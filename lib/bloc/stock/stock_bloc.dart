...
class MarketBloc extends Bloc<MarketEvent, MarketState> {
  final LoadMarketUseCase _loadMarketUseCase = getIt<LoadMarketUseCase>(); // ← Load raw data
  ...
  Future<void> _onInitialize(InitializeMarketEvent event, Emitter<MarketState> emit) async {
    final allStocks = _loadMarketUseCase();
    final savedMarketState = await _loadMarketStateUseCase.execute();

    String? selectedCategory;
    Map<String, List<String>> filterMap;
    if (savedMarketState != null && savedMarketState['selectedCategory'] != null) {
      ...
      selectedCategory = 'HOSE';
      filterMap =
      Map<String, List<String>>.from(MarketFilter.DEFAULT_FILTER_MAP);
    }
    emit(state.copyWith(
      allStocks: allStocks,
  ...
  Future<void> _onLoadMarket(MarketEventLoadMarket event, Emitter<MarketState> emit) async {
    final filteredStocks = await _filterMarketDataUseCase.execute(
      state.allStocks,
      state.selectedCategory ?? 'HOSE',
      state.filterMap ?? {},);
    emit(state.copyWith(stocks: filteredStocks, isLoading: false));
  }
