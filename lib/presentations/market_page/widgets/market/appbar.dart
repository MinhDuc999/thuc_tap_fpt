...
class CustomAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const CustomAppBar({super.key, required this.allMarket, required this.filterMarket});
  ...
  Widget build(BuildContext context) {
    return BlocListener<CatalogBloc, CatalogState>(
        listenWhen: (previous, current) => previous.filterCatalog != current.filterCatalog && current.selectedCatalog != null,
        listener: (context, catalogState) {
          context.read<MarketBloc>().add(MarketEventFilterByCategory(catalogState.selectedCatalog!, catalogState.filterCatalog),
          );
        },
              ...
              builder: (context, state) {
                return BlocBuilder<CatalogBloc,CatalogState>(
                    builder: (context,catalogState) {
                      final allCatalog = catalogState.allCatalog;
                      return Container(
                                      ...
                                      child: SingleChildScrollView(
                                        ...
                                        child: Row(
                                          children: allMarket.map((market) {
                                            ...
                                            return buildMenuButton(context, market.name, hasSubmenu, isSelected,
                                                    () {
                                                    //if (!hasSubmenu) {
                                                      ...
                                                      return
                                                        MenuItemButton(
                                                          onPressed: () {
                                                            ...
                                                            final currentStocks = catalogState.filterCatalog[currentCatalog] ?? [];
                                                            if (currentStocks.contains(stock.symbol)) {
                                                            ...
                                                            context.read<CatalogBloc>().add(AddStockToCatalogEvent(currentCatalog, stock.symbol));
                                                            Fluttertoast.showToast(
                                                            ...
                                                            //   context.read<MarketBloc>().add(MarketEventFilterByCategory(currentCatalog, newState.filterCatalog));
                                                            // });
                                                          },
                                                          style: ButtonStyle(
                                                            backgroundColor: WidgetStateProperty.all(Colors.transparent),
